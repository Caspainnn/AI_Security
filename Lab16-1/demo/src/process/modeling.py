...
def predict(step, test_loader_step):
    ...
    with torch.no_grad():
        ...
        for i, (output, label) in enumerate(zip(stats.outs(), stats.labels())):
            ...
            if predicted_probabilities > 0.5:
                ...
                predicted_label = 0 
            true_label = label
            print(f"Sample {i}: Predicted Probability: {predicted_probabilities}, Predicted Label: {predicted_label}")
