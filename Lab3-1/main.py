...
class AlexNet(nn.Module):
    def __init__(self):
        ...
        # 由于MNIST 为28x28,而最初AlexNet 的输入图片是227x227 的，所以网络层数和参数需要调
...
def fetch_datasets(full_dataset, trainset, ratio):
    ...
    for i, data in enumerate(character):
        num_poison_train_inputs = int(len(data) * ratio)
        ...
        for img in data[num_poison_train_inputs:]:
            ...
            clean_trainset.append((img, label))
...
trainset_all = datasets.MNIST(root='../data', download=True, train=True, transform=transform)
all_trainset = select_subset(trainset_all)

# 获取测试集
clean_test_all = datasets.MNIST(root='../data', download=True, train=False, transform=transform)
clean_test = select_subset(clean_test_all)
# 数据已经由 transform 转为 tensor 且归一化，直接保留
clean_testset = [(img, label) for img, label in clean_test]
...
for epoch in range(epoch):
    ...
    for index, (imgs, labels) in enumerate(trainset_dataloader, 0):
        # 获取输入数据 (datasets.ToTensor 已经给出 [B, C, H, W])
    ...
    # 在循环体外部测试样本准确率
    print("测试每一轮干净样本准确率: Epoch", epoch + 1)
    clean_correct = 0
