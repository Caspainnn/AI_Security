...
# ==============================
# 【Windows 必加】全部代码放进主函数
# ==============================
if __name__ == '__main__':

    if torch.cuda.is_available():
        device = torch.device("cuda:0")
        print("✅ CUDA 已启用，使用 GPU 训练")
    else:
    ...
    parser.add_argument('--workers', type=int, help='number of data loading workers', default=0)  # 强制改为0
    ...
    testloader = torch.utils.data.DataLoader(testset, batch_size=500,
                                             sampler=sp.SubsetRandomSampler(data_list), num_workers=0)  # 强制0
