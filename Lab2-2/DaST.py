...
parser.add_argument('--save_folder', type=str, default='saved_model', help='save folder')
...
if opt.dataset == 'azure':
    ...
    adversary_ghost = LinfBasicIterativeAttack(
        ...
        nb_iter=100, eps_iter=0.01, clip_min=0.0, clip_max=1.0, targeted=False
    )
    nc = 1
...
for epoch in range(opt.niter):
        ...
        if ii % 40 == 0:
            print('[%d/%d][%d/%d] D: %.4f D_prob: %.4f G: %.4f D(G(z)): %.4f / %.4f loss_imitate: %.4f loss_diversity: %.4f'
                  % (epoch, opt.niter, ii, batch_num,
                     errD.item(), errD_prob.item(), errG.item(), D_G_z1, D_G_z2,
    ...
    if best_att < (total - correct_ghost):
        torch.save(netD.state_dict(), os.path.join(opt.save_folder, f'netD_epoch_{epoch}.pth'))
        torch.save(netG.state_dict(), os.path.join(opt.save_folder, f'netG_epoch_{epoch}.pth'))
