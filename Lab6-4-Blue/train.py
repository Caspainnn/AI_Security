...
from data_caculate import data_root_path,load_vocab
from data_caculate import lab_path
...
def train(model):
    ...
    for epoch in range(3):
        for batch_id,data in enumerate(train_loader):
            ...
            # paddle.metric.accuracy() 内部期望输入格式是： label.shape = [batch_size, 1],所以需要用 unsqueeze(1) 增加一个维度
            if batch_id % 50 == 0:
                Iters.append(steps)
        ...
        model.train()
    paddle.save(model.state_dict(), lab_path+"model_final.pdparams")
